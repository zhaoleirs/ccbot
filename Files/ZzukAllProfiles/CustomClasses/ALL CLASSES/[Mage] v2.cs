    ...
    // This CC is not finished and since I do not play mage anymore I will not update it
    // It's is completly based on the work of people from ZZukbot, mainly emu and others
    // Please don#t be mad at me if I do not give credit to everyone simply because its an abandoded project and I do not remember all the names.
    ...
    namespace MacPod
    {
       ...
       public class Macpod : CustomClass
       {
          ...
          public override void Fight()
          {
             ...
             /*
             //Blink on CC
             if (this.Player.GotDebuff = "Cheap Shot" || this.Player.GotDebuff = "Kidney Shot" || this.Player.GotDebuff = "Intercept Stun" || this.Player.GotDebuff = "Hammer of Justice" || this.Player.GotDebuff = "Frost Nova" || this.Player.GotDebuff = "Stun")
             {
             ...
             */
             ...
             //Mana Shield
             if (this.Player.HealthPercent <= 30 && !this.Player.GotBuff("Mana Shield") && this.Player.ManaPercent >= 25 && useManaShield)
             {
             ...
             */
             ...
             if (this.Target.GotDebuff("Frost Nova") && this.Target.DistanceToPlayer <= 5)
             {
                //Use Blink if low hp
                if (this.Player.CanUse("Blink") && useBlink && this.Player.HealthPercent <= 40)
                {
                   this.Player.Cast("Blink");
                }
                else
                {
                   bool res = Player.ForceBackup(7);
                }
             }
             else
             {
             ...
             */
             ...
             if (this.Player.GetSpellRank("Cone of Cold") != 0 && this.Target.DistanceToPlayer <= 10 && this.Target.HealthPercent < 15 && this.Attackers.Count > 1)
             {
             ...
             if (this.Player.GetSpellRank("Fire Blast") != 0 && this.Target.DistanceToPlayer <= 20 && this.Target.HealthPercent < 15)
             {
             ...
             //Frost Nova
             if (this.Player.CanUse("Frost Nova") && this.Target.DistanceToPlayer <= 5 && this.Target.HealthPercent >= 28 && !this.Target.GotDebuff("Frost Nova") && !this.Target.GotDebuff("Frostbite"))
             {
                this.Player.Cast("Frost Nova");
                return;
             }
             //Coldsnap if Frost Nova is on CD and HP is low
             else if (this.Player.CanUse("Frost Nova") == false && this.Target.DistanceToPlayer <= 5 && this.Target.HealthPercent >= 28 && this.Player.CanUse("Cold Snap") && this.Player.HealthPercent <= 40 && !this.Target.GotDebuff("Frost Nova"))
             {
          ...
          public override bool Buff()
             ...
             if (this.Player.ItemCount("Mana Jade") == 0)
             {
                if(this.Player.TryCast("Conjure Mana Jade") && this.Player.ItemCount("Mana Jade") == 0)
                {
