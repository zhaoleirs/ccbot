...
namespace DoctorDeath
{
    internal static class Constants
    {
        public static readonly Version Release = new Version(0, 1);

        public const string Name = "DoctorDeath";
        public const byte Class = PlayerClass.Warlock;

        public static readonly string[] TalentStrings =
        {
            "05000505000000000000000000000000000000000000000000",
            "45002505002010000000000000000000000000000000000000",
            "55002505002010050005030000000000000000000000000000",
            "55002505002010050005230005050000000000000000000000"
        };
    }
    ...
    public class Core : CustomClass
    {
        ...
        {	
		/*
		if (!this.Pet.IsAlive() && this.Player.CanUse("Fear") && !this.Target.GotDebuff("Fear") && (CreatureType() != "Elemental") && CreatureType() != "Elemental")
		{
			this.Player.CastWait("Fear", 3000);
		}	
		if (!this.Pet.IsAlive() && this.Player.CanUse("Summon Voidwalker") && this.Target.GotDebuff("Fear") && this.Player.ManaPercent > 50)
		{
		...
		*/
        ...
        public override bool Buff()
        {
            try
            {
                ...
                if (this.IsCastingSpell(Spellbook.SummonVoidwalker) || this.IsCastingSpell(Spellbook.SummonImp))
                {
                    return false;
                }
                ...
                if (Spellbook.SummonVoidwalker.IsWanted && this.CanCast(Spellbook.SummonVoidwalker))
                {
                    this.Cast(Spellbook.SummonVoidwalker);
                }
                else if (Spellbook.SummonImp.IsWanted && this.CanCast(Spellbook.SummonImp))
                {
                    this.Cast(Spellbook.SummonImp);
                }
			...
			if (this.Player.CanUse("Life Tap") && this.Player.ManaPercent < 80 && this.Player.HealthPercent > 70)
                {
        ...
        public override void Rest()
        {
			...
			if ((this.Player.GotBuff("Drink") || this.Player.GotBuff("Food")) && this.Pet.IsAlive() && this.Pet.HealthPercent < 60)
			{
				if (this.Player.CanUse("Consume Shadows"))
				{
					this.Player.CastWait("Consume Shadows", 10000);
				}
			}	
            base.Rest();
        }
        ...
        private bool HandledMultiTargetCombat()
        {
            ...
            {
                if (Target.GotDebuff(Spellbook.CurseOfAgony.ToString()) &&
                    Target.GotDebuff(Spellbook.Corruption.ToString()))
                {
                    var unitToAttack =
                        Attackers.FirstOrDefault(
                            a =>
                                !a.GotDebuff(Spellbook.CurseOfAgony.ToString())
                                ||
                                !a.GotDebuff(Spellbook.Corruption.ToString()));

                    if (unitToAttack != null)
                    {
                        Player.StopWand();
                        Player.StopAttack();
                        Player.SetTargetTo(unitToAttack);
                        Pet.Attack();
                        FixDotOnTarget(unitToAttack, Spellbook.CurseOfAgony);
                        FixDotOnTarget(unitToAttack, Spellbook.Corruption);
                        return false;
                    }
        ...
        private void FixDotOnTarget(_Unit unitToAttack, Spell dotSpell)
        {
            if (this.CanCast(dotSpell) && !unitToAttack.GotDebuff(dotSpell.ToString()))
            {
                this.Cast(dotSpell);
            }
        }
        ...
        {
            var lifetap = Spellbook.LifeTap;

            if (lifetap.IsWanted && this.CanCast(lifetap))
            {
                this.Cast(lifetap);
            }

            var wand = Spellbook.Wand;

            if (wand.IsWanted && this.CanCast(wand))
            {
                wand.Cast();
            }

            var shoot = Spellbook.Attack;

            if (shoot.IsWanted && this.CanCast(shoot))
            {
                shoot.Cast();
            }
        }

        private void TakeCareOfOurPet()
        {
            this.Pet.Attack();

            var healthFunnel = Spellbook.HealthFunnel;

            if (healthFunnel.IsWanted && this.CanCast(healthFunnel))
            {
                this.Cast(healthFunnel);
            }

            if ((this.Player.HealthPercent < 10 || this.Pet.HealthPercent <= 6) && !this.Player.GotBuff("Sacrifice"))
            {
                this.Pet.Cast("Sacrifice");
            }
    ...
    internal class Spellbook
    {
		...
		public static readonly Spell SiphonLife = new Spell("Siphon Life", 798, false, true, isInstant: true,
        isWanted: () => Target.HealthPercent > 80 && !Target.GotDebuff("Siphon Life") && Me.CanUse("Siphon Life") && Me.ManaPercent >= 40 && Me.HealthPercent <= 92);
		
        public static readonly Spell Corruption = new Spell("Corruption", 800, true, true, isInstant: true,
            isWanted: () => Target.HealthPercent > 20 && !Target.GotDebuff("Corruption") && Me.CanUse("Corruption"));

        public static readonly Spell CurseOfAgony = new Spell("Curse of Agony", 799, true, true, isInstant: true,
            isWanted:
                () => Target.HealthPercent > 35 && !Target.GotDebuff("Curse of Agony") && Me.CanUse("Curse of Agony"));

        public static readonly Spell DrainSoul = new Spell("Drain Soul", 950, false, true, isChanneled: true,
            isWanted:
                () =>
                    Target.HealthPercent < 15 && Me.HealthPercent > 50 && Me.ItemCount("Soul Shard") < 5 &&
                    Me.CanUse("Drain Soul"));

        public static readonly Spell DrainLife = new Spell("Drain Life", 951, false, true, true, isChanneled: true,
            isWanted: () => Me.HealthPercent < 100 && Me.CanUse("Drain Life") && Target.DistanceToPlayer <= 20 && !Me.GotBuff("Shadow Trance"));

        // Affliction misc spells.
        public static readonly Spell LifeTap = new Spell("Life Tap", 949, false, true, isInstant: true,
            isWanted: () => Me.ManaPercent < 80 && Me.HealthPercent > 75 && Me.CanUse("Life Tap"));

        // Demonology buff spells.
        public static readonly Spell DemonSkin = new Spell("Demon Skin", 1, true, false,
            isWanted:
                () =>
                    Me.GetSpellRank("Demon Armor") <= 0 && !Me.GotBuff("Demon Armor") && !Me.GotBuff("Demon Skin") &&
                    Me.CanUse("Demon Skin"));

        public static readonly Spell DemonArmor = new Spell("Demon Armor", 2, true, false,
            isWanted:
                () =>
                    Me.GetSpellRank("Demon Armor") > 0 && !Me.GotBuff("Demon Armor") && !Me.GotBuff("Demon Skin") &&
                    Me.CanUse("Demon Armor"));

        // Demonology misc spells.
        public static readonly Spell SummonVoidwalker = new Spell("Summon Voidwalker", 1, false, false,
            isWanted:
                () =>
                    (Me.GotPet() == false || Pet.HealthPercent <= 0) && Me.GetSpellRank("Summon Voidwalker") >= 1 &&
                    Me.ItemCount("Soul Shard") > 0 && Me.CanUse("Summon Voidwalker"));

        public static readonly Spell SummonImp = new Spell("Summon Imp", 2, false, false,
            isWanted:
                () =>
                    (Me.GotPet() == false || Pet.HealthPercent <= 0) && Me.GetSpellRank("Summon Imp") >= 1 &&
                    Me.ItemCount("Soul Shard") <= 0 && Me.CanUse("Summon Imp"));

        public static readonly Spell HealthFunnel = new Spell("Health Funnel", 3, true, false, true, true,
            isWanted:
                () =>
                    Me.GotPet() && Pet.HealthPercent > 0 && Pet.HealthPercent < 40 && Me.HealthPercent > 60 && !Me.GotBuff("Drink") && !Me.GotBuff("Food") && Target.GotDebuff("Corruption") && Target.GotDebuff("Curse of Agony"));

        // Destruction damage spells.
        public static readonly Spell ShadowBolt = new Spell("Shadow Bolt", 952, false, true,
            isWanted: () => (Me.CanUse("Shadow Bolt") && Me.GotBuff("Shadow Trance") && Target.HealthPercent <= 85) ||
					(Me.CanUse("Shadow Bolt") && Target.GotDebuff("Corruption") && Target.GotDebuff("Curse of Agony") && Target.GotDebuff("Immolate") && Me.HealthPercent > 99 && Target.HealthPercent >= 50) || (Me.CanUse("Shadow Bolt") && Target.DistanceToPlayer > 21));
			

        public static readonly Spell Immolate = new Spell("Immolate", 798, false, true,
            isWanted:
                () =>
                    Target.HealthPercent >= 65 && !Target.GotDebuff("Immolate") && lastSpell.Priority != 798 &&
                    Me.CanUse("Immolate"));

        // Misc
        public static readonly Spell Wand = new Spell("Shoot", 1200, false, false, true, true, isWanted: () =>
        {
            return Me.IsWandEquipped() && Me.ManaPercent < 20 &&
                   Me.CanUse("Shoot");
        }, customAction: () => Me.DoString("if not IsAutoRepeatAction(1) then CastSpellByName(\"Shoot\"); end"));
		
//		public static readonly Spell Wand = new Spell("Shoot", 1200, false, false, true, true, isWanted: () =>
//        {
//            return Me.IsWandEquipped() && (Target.HealthPercent > 20 && !Target.GotDebuff("Corruption")) == false &&
//                   (Target.HealthPercent > 35 && !Target.GotDebuff("Curse of Agony")) == false &&
//                   Me.CanUse("Shoot");
//        }, customAction: () => Me.DoString("if not IsAutoRepeatAction(1) then CastSpellByName(\"Shoot\"); end"));
		

        public static readonly Spell Attack = new Spell("Attack", Int32.MaxValue, false, true, true, false,
            () => !Me.IsWandEquipped() && Me.CanUse("Attack"), customAction:
                () =>
                {
                    Me.Attack();
                    customClassInstance.Value.SetCombatDistance(3);
                });
